// 1. 给定一个括号表达式，只有[]和()，判断表达式括号是否平衡，比如[(())]返回true,[()(()])不平衡


const signs = {'[': ']', ']': '[', '(': ')', ')': '(' }
const keys = Object.keys

function nagate(cur) {
  return signs[cur]
}

function parse(str) {
  const stack = []
  let i = 0, cur
  while(cur = str.charAt(i)) {
    if (stack[stack.length - 1] === nagate(cur)) {
      stack.pop()
    } else if (keys(signs).includes(cur)) {
      stack.push(cur)
    }
    i++
  }
  return !stack.length
}

function parse1(str) {
  const stack = str.split('').reduce((last, next) => {
    if (last[last.length - 1] === nagate(next)) {
      last.pop()
    } else if (keys(signs).includes(next)) {
      last.push(next)
    }
    return last
  }, [])
  return !stack.length
}

function parse2(str) {
  const [first, ...others] = str
  const stack = [first]
  while(others.length) {
    const last = stack[stack.length - 1]
    const next = others.shift()
    if (last === nagate(next)) {
      stack.pop()
    } else if (keys(signs).includes(next)) {
      stack.push(next)
    }
  }
 
  return !stack.length
}

console.log(parse('[(a(1,2,4)3)]'))
console.log(parse('[(a(1,2,4)3)]('))
console.log(parse1('[(a(1,2,4)3)]'))
console.log(parse1('[(a(1,2,4)3)]('))
console.log(parse2('[(a(1,2,4)3)]'))
console.log(parse2('[(a(1,2,4)3)]('))